"""
Run the BM25-only RAG pipeline on BioASQ data

Usage:
  python scripts/run_bioasq_pipeline_bm25.py --round 1 --email user@example.com --config configs/pipeline_config.yaml --data-dir data --output results --max-questions 10
"""
...
def run_pipeline(questions: List[Dict[str, Any]], documents: List[Dict[str, Any]], config_path: str) -> List[Dict[str, Any]]:
    logger.info("Initializing BM25-only RAG pipeline...")
    import yaml
    with open(config_path, "r") as f:
        config = yaml.safe_load(f)
    pipeline = MedicalRAGPipelineBM25(config)
...
def evaluate_results(predictions: List[Dict[str, Any]], golden_data: Dict[str, Any], use_llm_judge: bool = False) -> Dict[str, float]:
    ...
    logger.info("Evaluating predictions (BM25)...")
...
def save_results(predictions: List[Dict[str, Any]], metrics: Dict[str, float], output_dir: str, round_num: int):
    ...
    pred_file = os.path.join(base_dir, "predictions_bm25.json")
    metrics_file = os.path.join(base_dir, "metrics_bm25.json")
    with open(pred_file, "w") as f:
        json.dump(predictions, f, indent=2)
    with open(metrics_file, "w") as f:
        json.dump(metrics, f, indent=2)
    logger.info(f"Saved BM25 predictions to {pred_file}")
    logger.info(f"Saved BM25 metrics to {metrics_file}")


def main():
    parser = argparse.ArgumentParser(description="Run BM25-only RAG pipeline on BioASQ data")
