"""
Run the BioBERT-dense RAG pipeline on BioASQ data

Usage:
  python scripts/run_bioasq_pipeline_biobert.py --round 1 --email user@example.com --config configs/pipeline_config.yaml --data-dir data/bioasq --output results --max-questions 10
"""
...
def run_pipeline(questions: List[Dict[str, Any]], documents: List[Dict[str, Any]], config_path: str) -> List[Dict[str, Any]]:
    logger.info("Initializing BioBERT RAG pipeline...")
    import yaml
    with open(config_path, "r") as f:
        config = yaml.safe_load(f)
    pipeline = MedicalRAGPipelineBioBERT(config)
    logger.info(f"Indexing {len(documents)} documents into FAISS (BioBERT embeddings)...")
...
def evaluate_results(predictions: List[Dict[str, Any]], golden_data: Dict[str, Any], use_llm_judge: bool = False) -> Dict[str, float]:
    ...
    logger.info("Evaluating predictions (BioBERT retriever)...")
...
def save_results(predictions: List[Dict[str, Any]], metrics: Dict[str, float], output_dir: str, round_num: int):
    ...
    pred_file = os.path.join(base_dir, "predictions_biobert.json")
    metrics_file = os.path.join(base_dir, "metrics_biobert.json")
    with open(pred_file, "w") as f:
        json.dump(predictions, f, indent=2)
    with open(metrics_file, "w") as f:
        json.dump(metrics, f, indent=2)
    logger.info(f"Saved BioBERT predictions to {pred_file}")
    logger.info(f"Saved BioBERT metrics to {metrics_file}")


def main():
    parser = argparse.ArgumentParser(description="Run BioBERT-dense RAG pipeline on BioASQ data")
